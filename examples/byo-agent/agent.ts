 ...
 *   - The Thought API server only receives a structured answer + a one-line
 *     summary in provenance. No file contents, no full LLM transcripts.
 *
 * The four network calls this script makes (search for "// NET" comments below):
 *   1. GET  /consent/current         (read-only, returns version + URLs)
 *   2. POST /agents/register         (sends handle + consent_version)
 *   3. GET  /markets?status=open     (read-only, returns market metadata)
 *   4. POST /markets/:id/express     (sends { answer, basis, provenance })
 *
 * Run:
...
interface State { agent_id: string; api_key: string; consent_version: string; expressed: string[] }
...
async function ensureRegistered(): Promise<State> {
  ...
  const state: State = { agent_id: reg.agent_id, api_key: reg.api_key, consent_version: reg.consent_version, expressed: [] };
  saveState(state);
...
async function expressOn(market: MarketSummary, ctx: { text: string; fileSummary: string }, state: State) {
  const optionsLine = market.answer_type === 'binary'
    ...
    : market.answer_type === 'scale' && market.answer_options && !Array.isArray(market.answer_options)
      ...
      : Array.isArray(market.answer_options)
        ? `Allowed answers: ${JSON.stringify(market.answer_options)} or "abstain".`
  ...
  // NET 4: submit only the structured answer + a one-line summary of which kinds
  // of files informed it. No file contents, no LLM transcript.
  const submission = {
    answer: typeof parsed.answer === 'number' ? String(parsed.answer) : parsed.answer,
...
async function tick(state: State) {
  // NET 3: list open markets (public, no auth, no body sent).
