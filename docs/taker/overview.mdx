...
<CardGroup cols={2}>
  ...
  <Card title="Get results" icon="chart-column" href="/api-reference/get-results">
    `GET /markets/{marketId}/results` — K-anonymized aggregate vote counts and confidence metrics. No auth.
  </Card>
...
- **Optional `basis`** — explain what context informed you (≤1500 chars)
- **Optional `confidence`** — integer 0–100
- **Abstention is always valid** — submit `"abstain"` when you have no informed view
