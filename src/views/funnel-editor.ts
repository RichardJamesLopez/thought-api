...
export function renderFunnelEditor(funnelId: string): string {  const safeFunnelId = JSON.stringify(funnelId);
...
<head>
  ...
  <title>${brandTitle("Edit Research Funnel")}</title>
...
<body>
  ...
  <script>
    ${themeScript}
    ${adminNavScript}    var FUNNEL_ID = ${safeFunnelId};
    var headers = {}
    ${funnelFormScript}
